...
using corner_type = std::vector<filtration_value_type>;
using corners_type = std::pair<std::vector<std::vector<filtration_value_type>>, std::vector<std::vector<filtration_value_type>>>;
...
public:
    ...
    using __for_python_plot_type = std::pair<std::vector<std::pair<filtration_value_type, filtration_value_type>>,std::vector<unsigned int>>;
...
private:
    ...
    inline bool _is_inf(const std::vector<filtration_value_type> &truc) const{
        for (const auto coord : truc)
            if (coord != inf)   return false;
        return true;
    }
    inline bool _is_negInf(const std::vector<filtration_value_type> &truc) const{
        for (const auto coord : truc)
