...
namespace Vineyard
{
	...
	public:
		...
		using get_pixel_value_function_type = std::function<double(const module_type::const_iterator, const module_type::const_iterator, double, double)>;
		...
		image_type get_vectorization_in_dimension(
			...
			unsigned int verticalResolution,
			get_pixel_value_function_type get_pixel_value) const;
		std::vector<double> get_landscape_values(
	...
	private:
		...
		void _compute_2D_image(image_type &image,
							   ...
							   unsigned int verticalResolution,
							   get_pixel_value_function_type get_pixel_value)const;
		double _get_pixel_value(
			const module_type::iterator start,
			const module_type::iterator end,
			const corner_type x, const double delta, const double p, const bool normalize, const double moduleWeight
		) const;
	};
	...
	public:
		...
		double get_interleaving() const;
		double get_interleaving(const Box &box);
		double get_local_weight(const corner_type &x, const double delta) const;
		std::pair<corner_type, corner_type> get_bar(const Line &line) const;
		void add_bar(
			double baseBirth,
			double baseDeath,
			const corner_type &basepoint,
		...
		double get_landscape_value(const std::vector<double>& x) const;
	...
	private:
		...
		double _rectangle_volume(const corner_type &a, const corner_type &b) const;
		double _get_max_diagonal(const corner_type &a, const corner_type &b, const Box &box) const;
		double d_inf(const corner_type &a, const corner_type &b) const;
...
#pragma omp parallel for
	...
	std::vector<double> Module::get_landscape_values(
		const std::vector<double>& x,
		const dimension_type dimension
	) const
	{
		...
		std::sort(out.begin(), out.end(), [](const double x, const double y){ return x>y;});
		return out;
	}
	Module::image_type Module::get_landscape(
		...
		Module::image_type image;
		image.resize(resolution[0], std::vector<double>(resolution[1]));
		double stepX = (box.get_upper_corner()[0] - box.get_bottom_corner()[0]) / resolution[0];
		double stepY = (box.get_upper_corner()[1] - box.get_bottom_corner()[1]) / resolution[1];
	...
	std::vector<Module::image_type> Module::get_landscapes(
		...
		for (unsigned int i = 0; i < resolution[0]; i++){
			for (unsigned int j = 0; j < resolution[1]; j++){
				std::vector<double> landscapes = this->get_landscape_values(
					{
						box.get_bottom_corner()[0] + stepX * i,
						box.get_bottom_corner()[1] + stepY * j
					}, dimension);
	...
	MultiDiagram Module::get_barcode(const Line &l, const dimension_type dimension, const bool threshold) const
	{
		...
		for (unsigned int i=0; i < this->size(); i++){
			...
			corner_type &pbirth = pushed_summand.first;
			corner_type &pdeath = pushed_summand.second;
			if (threshold){
				auto &min = threshold_bounds.first;
				auto &max = threshold_bounds.second;
				if (is_smaller(pbirth, min)) pbirth = min;
				else if (is_greater(pbirth, max)) {pbirth = corner_type(l.get_dim(), inf); pdeath = pbirth;}
				if (is_greater(pdeath, max) && pbirth[0] != inf) pdeath = max;
				else if (is_smaller(pdeath, min)) {pbirth = corner_type(l.get_dim(), negInf); pdeath = pbirth;}
			}
	...
	MultiDiagrams Module::get_barcodes(const std::vector<point_type> &basepoints, const dimension_type dimension, const bool threshold)const {
		...
		MultiDiagrams out(nlines);
		for (unsigned int i = 0; i < nlines; i++){
			const Line &l = Line(basepoints[i]);
			out[i] = this->get_barcode(l, dimension, threshold);
		}
		return out;
	}
...
#pragma omp parallel for collapse(2)
	...
	inline double Module::_get_pixel_value(
		...
		const double p,
		const bool normalize,
		const double moduleWeight)const
	{
		double value = 0;
		if (p == 0){
...
#pragma omp parallel for reduction(std::max : value)
	...
	inline double Summand::get_interleaving() const
	{
		return distanceTo0_;
	}

	inline double Summand::get_local_weight(const corner_type &x, const double delta) const
	{
...
#pragma omp simd
	...
	inline std::pair<corner_type, corner_type> Summand::get_bar(const Line &l) const{
		corner_type pushed_birth(this->get_dimension(), inf);
		corner_type pushed_death(this->get_dimension(), negInf);
		for (const corner_type &birth : this->get_birth_list()){
			corner_type pb = l.push_forward(birth);
			if (is_smaller(pb, pushed_birth)) pushed_birth.swap(pb);
		}
		//
		for (const corner_type &death : this->get_death_list()){
			corner_type pd = l.push_back(death);
			if (is_greater(pd, pushed_death)) pushed_death.swap(pd);
		}
		if (!is_smaller(pushed_birth, pushed_death))
			return {corner_type(this->get_dimension(), inf), corner_type(this->get_dimension(), inf)};
...
#pragma omp simd
		for (unsigned int j = 0; j < birth.size() - 1; j++)
		{
			double temp = basepoint[j] + baseBirth;
	...
	inline double Summand::get_landscape_value(const std::vector<double>& x) const{
		double out = 0;
		Box trivial_box;
		for (const corner_type& b : this->birth_corners_){
			for (const corner_type& d : this->death_corners_){
				double value = std::min(this->_get_max_diagonal(b,x, trivial_box), this->_get_max_diagonal(x,d, trivial_box));
...
#pragma omp parallel for reduction(max \
  ...
  //
	...
	inline double Summand::_get_max_diagonal(const corner_type &birth, const corner_type &death, const Box &box) const
	{
		if constexpr (Debug::debug)
	...
	inline double Summand::_rectangle_volume(const corner_type &a, const corner_type &b) const
	{
		if constexpr (Debug::debug)
	...
	inline double Summand::d_inf(const corner_type &a, const corner_type &b) const
	{
