...
if "self_test" in argv:
    ...
    for _pipeline in ("multismk", "multismi", "multisurface", "mmaimg"):
        ...
        assert (
            system(
                f"python {script_name} --dataset {easy_graph} "
        ...
        assert (
            system(
                f"python {script_name} --dataset {easy_graph} --filtrations degree "
                ...
                f"--out_strategy regular_closest --out_resolution 5 --n_jobs {n_jobs} --test 1"
            )
            == 0
        )
        ...
        for _complex in ("rips", "alpha"):
            for _kernel in ["gaussian"]:
                assert (
                    system(
                        f"python {script_name} --dataset {easypoint_cloud} "
                        ...
                        f"--dtm_masses .1 --n_jobs {n_jobs} --num_pts 50 --num_samples 5 --test 1"
                    )
                    == 0
                )
...
p.add_argument(
    ...
    type=str,
    required=True,
    help="The dataset on which to do the computation. Either UCR, e.g., UCR/Coffee, graphs : graphs/BZR, orbit, 3dshapes, e.g., 3dshapes/Airplane ",
)  # Threshold infinite values to compute diagram distance
...
p.add_argument(
    ...
    required=True,
    type=str,
    help="The pipeline to apply to the dataset. Available : dummy, filvec, pervec, sw, {rd,dr,multi}_{smi, smk, hilbert}, sw, smk, smh, smi, pl,pl_p, pi, pi_p. Where sm -> signed measure, sm{i,k,h} -> image, kernel, hilbert, rd -> rips+density bifiltration, dr-> degree+rips bifiltration, multi -> custom 1critical multi filtration (eg. graphs, molecules), sw -> sliced wasserstein, pl -> persistence landscape, pi -> persistance image.",
)  # pipeline
p.add_argument(
    "-fc",
    "--final_classifier",
    default="rf",
    type=str,
    help="When the final input is a vector, this defines the final classifier",
)
p.add_argument(
    "-f",
    "--filtration",
    default="",
    type=str,
    help="For 1 parameter filtration, the custom filtration. for example for graphs : ricciCurvature",
)  # filtration on the graph (1-parameter)
p.add_argument(
    "-fs",
    "--filtrations",
    default=[],
    type=str,
    action="append",
    help="For multifiltration, the filtrations to consider. e.g. --filtrations ricciCurvature --filtrations cc --filtrations geodesic. Depending on the dataset, available ones for graphs are cc,degree,fiedler,ricciCurvature,geodesic.",
)  # filtrations on the graph (multi-parameter)
p.add_argument(
    "-tk",
    "--train_k",
    default=10,
    type=int,
    help="Number of cross validations to choose the parameters during the training",
)  # number of kfold for cross validation
p.add_argument(
    "-k",
    "--test_k",
    default=10,
    type=float,
    help="Number of n-folds for testing. If 0<x<1, will do a train-test-split with a proportion of x for the test.",
)  # number of kfold for test
...
p.add_argument(
    ...
    default=np.inf,
    type=float,
    help="For 1 parameter, thresholds persistence values to this threshold.",
)  # Threshold infinite values to compute diagram distance
p.add_argument(
    "-ns",
    "--num_samples",
    default=-1,
    type=int,
    help="number of data for orbit5k, and 3dshapes",
)  # number of data (e.g. graph, or orbit data) samples
p.add_argument(
    "-npts",
    "--num_pts",
    default=0,
    type=int,
    help="number of points / nodes in each data for 3dshapes / orbit",
)  # number of pts per sample, if (synthetic)
p.add_argument(
    "-res",
    "--in_resolution",
    type=int,
    default=None,
    help="For multiparameter pipelines, the resolution to compute the signed measure. e.g. 100 will do the computation on a [100]*num_parameter grid.",
)
...
p.add_argument(
    ...
    "--in_strategy",
    default="exact",
    help="Infers the grid on which to compute the topological invariant.",
)
p.add_argument(
    "--in_individual_grid",
    default=True,
    type=int,
    help="Whether or not to compute signed meaasure on individual grids. Significantly faster if true.",
)
...
p.add_argument(
    ...
    nargs="+",
    action="extend",
    help="Infers the grid on which to compute the topological invariant. Available : regular, quantile, exact.",
)
...
p.add_argument(
    ...
    type=int,
    action="append",
    help="For multiparameter, vectorized pipelines, e.g. *_{smi, hilbert} the resolution of these vectors.",
)
...
p.add_argument(
    ...
    "--complex",
    default="rips",
    help="Simplicial complex used on the point cloud.",
)
p.add_argument(
    "-krnl",
    "--kernel",
    default="gaussian",
    help="Codensity-like kernel to use, e.g., gaussian, exponential.",
)
...
p.add_argument(
    ...
    action="append",
    default=[],
    help="For point cloud dataset, the bandwidths of the Kernel Density Estimation to cross validate.",
)
p.add_argument(
    "--dtm_masses",
    type=float,
    action="append",
    default=[],
    help="For point cloud dataset, the selected masses of the DistanceToMeasure to cross validate to compute a codensity filtration.",
)

p.add_argument(
    "--drop_quantile",
    default=0,
    help="When inferring the filtrations, drop filtration values lower than this q and greater than 1-q.",
    type=float,
)
p.add_argument(
    "--num_rescales",
    default=1,
    help="Number of rescales per filtration for Kernel.",
    type=int,
)
...
p.add_argument(
    ...
    type=float,
    default=np.inf,
    help="Maximum radius value for rips, when using a pipeline using rips.",
)
...
p.add_argument(
    ...
    type=float,
    default=None,
    help="Value of the sparse rips, if using it.",
)
p.add_argument(
    "--extended",
    action="append",
    type=int,
    default=[],
    help="Extended persistence for 1 parameter filtrations. if -1 : will use [0,2,5,7]. (Order given by gudhi)",
)  # TODO remove
...
p.add_argument(
    ...
    default=None,
    type=int,
    help="Some pipeline have randomized fit, this controls their seed.",
)  # node selection seed
p.add_argument(
    "--test",
    default=False,
    type=bool,
    help="Reduces the number of input, to ensure the pipelines are working. DO NOT USE WHEN NOT TESTING.",
)
...
p.add_argument(
    ...
    action="append",
    type=int,
    help="The homological degrees to consider. Note : `None` represent the euler characteristic.",
)
p.add_argument(
    "--rank_degrees",
    action="append",
    type=int,
    help="The homological degrees, for the rank invariant, to consider. ",
)


p.add_argument(
    "--n_jobs", default=cpu_count(), type=int, help="The number of threads to use."
)
...
p.add_argument(
    ...
    default=False,
    type=bool,
    help="Computes small tests to ensure the script works.",
)
args = p.parse_args()
args.DATASET_PATH = expanduser(args.DATASET_PATH)
np.random.seed(args.seed)

args.degrees = (
    [] if args.degrees is None else [None if d < 0 else d for d in args.degrees]
)
args.rank_degrees = [] if args.rank_degrees is None else args.rank_degrees
assert len(args.degrees) > 0 or len(args.rank_degrees) > 0, (
    "Provide homological degree to compute."
)
...
args.grid_shape = (
    None if args.in_resolution is None else [args.in_resolution] * num_parameters
)
...
SM_parameters = {
    ...
    "expand": args.complex == "rips" and "graphs" not in args.dataset,
...
if args.dataset == "orbit":
    ...
    SM_parameters["enforce_null_mass"] = (
        "smk" in args.pipeline and args.rips_threshold not in [np.inf, -1]
    )
elif args.dataset.startswith("UCR/"):
    from multipers.data import UCR

    xtrain, ytrain = UCR.get_train(dataset=args.dataset, DATASET_PATH=args.DATASET_PATH)
    xtest, ytest = UCR.get_test(dataset=args.dataset, DATASET_PATH=args.DATASET_PATH)
    ## FOR ACCURACY 2 CSV
    ...
    _xtrain, _xtest, _ytrain, _ytest = train_test_split(
        X, Y, shuffle=shuffle, test_size=args.test_k
    )
    ...
    X, Y = immuno_regions.get(DATASET_PATH=args.DATASET_PATH)
    SM_parameters["enforce_null_mass"] = (
        "smk" in args.pipeline and args.rips_threshold not in [np.inf, -1]
    )
    ...
    X, Y = shape3d.get(
        dataset=args.dataset, num_graph=args.num_samples, node_per_graph=args.num_pts
    )
    args.kde_bandwidths = []
    args.dtm_masses = []
    ...
    for f in filtrations:
        graphs.get(
            dataset=args.dataset, filtration=f
        )  # Ensures that the filtration f is computed on this dataset
...
print(SM_parameters)
print("------------- ")
STM2SM = mms.SimplexTree2SignedMeasure(**SM_parameters)
STMs2SMs = mms.SimplexTree2SignedMeasure(**SM_parameters)
...
SMDs = mms.SignedMeasures2SlicedWassersteinDistances(
    ...
    progress=True,
    scales=None
    if args.num_rescales <= 1
    else get_filtration_weights_grid(
        num_parameters=num_parameters,
        weights=np.unique([1.0] + list(np.linspace(0.1, 10.0, args.num_rescales - 1))),
...
print(
    f"Number of axis : bandwidths ({num_bandwidth}) x num_scales ({num_kernel_rescale}) = {num_axes}",
    flush=1,
)
...
MMAF_parameters = {
    ...
    if args.num_rescales <= 1
        ...
        num_parameters=num_parameters,
        weights=np.unique([1.0] + list(np.linspace(0.5, 10.0, args.num_rescales - 1))),
        remove_homothetie=False,
    ),
...
class Identity(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        return [
            np.asarray([np.asarray(stuff).flatten() for stuff in x]).flatten()
            for x in X
        ]
        # return X
...
if "surface" in args.pipeline:
    ...
    SMF_parameters["SMF__filtrations_weights"] = get_filtration_weights_grid(
        num_parameters=num_parameters,
        weights=np.unique([1.0] + list(np.linspace(0.01, 1.0, args.num_rescales - 1))),
        remove_homothetie=False,
    )
...
if "smk" in args.pipeline:
    ...
    DM2K_parameters = {
        "DM2K__sigma": [
            ...
            100,
            1000,
        ],  # TODO : With measures between 0,1 distances should also be very small ...
        "DM2K__axis": list(
            range(num_axes)
        ),  # if args.pipeline == "rd_smk" else [None], # Only for rips+density
        "DM2K__weights": get_filtration_weights_grid(
            num_parameters=len(SM_parameters["degrees"])
            + len(SM_parameters["rank_degrees"]),
            weights=[1, 0.1, 10],
        ),
    }
...
if args.dataset.startswith("3dshapes/"):
    ...
    ToSimplexTree = mmo.TorchData2DijkstraSimplexTree(
        true_geodesic=true_geodesic, progress=True
    )  # dtype=None delays the computation for multithread with simplextrees
    ToSimplexTreeMulti = None  # TODO ?
    ToSignedMeasure = None
    SMD = None
elif args.dataset in ["orbit", "immuno"] or args.dataset.startswith(
    "UCR/"
):  # point clouds
    import multipers.ml.filtered_complex as mmp

    collapse_num = -2 if args.complex == "rips" else 0
    if args.complex == "delaunay":
        collapse_num = None
    ...
    RDs2STs = Pipeline(
        [
            (
                "st",
                mmp.PointCloud2FilteredComplex(
            ...
            (
                "pre",
                mmp.FilteredComplexPreprocess(
                    num_collapses=collapse_num, n_jobs=args.n_jobs
                ),
            ),
        ]
    )
    ...
    SMD = SMDs
    assert num_bandwidth > 0 or "one" in args.pipeline, (
        "Need a bandwidth/mass parameter to compute a codensity axis."
    )
...
compute_diagram_pipe = Pipeline(
    [
        ...
        (
            ...
            mmo.SimplexTree2Dgm(
                n_jobs=args.n_jobs,
                threshold=args.diagram_threshold,
                extended=extended,
                degrees=degrees,
                progress=True,
            ),
...
match to_switch:
            ...
            [
                ...
                (
                    ...
                    mma.MMAFormatter(
                        ...
                        verbose=True,
                        degrees=int_degrees,
                        quantiles=[args.drop_quantile] * 2,
                    ),
                ),
            ]
        )
    ...
    case "filvec":
        ...
        pipeline = Pipeline(
            [("st", ToSimplexTree), ("hist", mmo.SimplexTree2Histogram()), ("svm", svm)]
        )
    case "msmi" | "multismi":
        print("------------smi pipeline")
        ### PREPROCESSING : transform to signed measure
        SignedMeasureTransformer = Pipeline(
            [("st", ToSimplexTreeMulti), ("sm", ToSignedMeasure)]
        )
    ...
    case "dr" | "degreerips":
        ...
        pipeline = Pipeline(
            [("SMM2CV", SMM2CV), ("final_classifier", final_classifier)]
        )
        parameters = {}
        parameters.update(SMM2CV_parameters)
        parameters.update(final_classifier_parameters)
    case "sw" | "slicedwasserstein":
        ...
        swds = mmo.Dgms2SlicedWassersteinDistanceMatrices(
            num_directions=10, n_jobs=args.n_jobs
        ).fit_transform(diagrams)
        # sanitize non-finite entries (NaN/inf) that can appear with degenerate
        # diagrams. Replace them with a finite value (max finite) so downstream
        # kernel/SVM code does not crash. This keeps smoke tests robust.
        swds = np.asarray(swds)
        if not np.isfinite(swds).all():
            finite = swds[np.isfinite(swds)]
            if finite.size == 0:
                replacement = 0.0
            else:
                replacement = finite.max()
            swds = np.nan_to_num(
                swds, nan=replacement, posinf=replacement, neginf=replacement
            )
        print("Formatting Distance Matrix", flush=True)
        # kernels.py exposes DistanceMatrices2DistancesList as DistanceMatrices2DistancesList
        # but previous import path was wrong in one module. Use the implementation from
        # multipers.ml.kernels directly to avoid accidental shadowing.
        from multipers.ml.kernels import DistanceMatrices2DistancesList

        X = DistanceMatrices2DistancesList().fit_transform(swds)
        # ensure local references exist (re-import multipers.ml.kernels as mmk in this scope)
        import multipers.ml.kernels as mmk

        DL2D = mmk.DistancesLists2DistanceMatrices()
        # DM2K may not have been created earlier (it is created when 'smk' is in the
        # pipeline); create a local instance here so the pipeline can be constructed
        # independently for the SW branch.
        DM2K = mmk.DistanceMatrix2Kernel()
        pipeline = Pipeline([("dms", DL2D), ("DM2K", DM2K), ("SVMP", SVMP)])
        parameters = {}
        # DM2K_parameters is defined for 'smk' pipelines; guard its usage here.
        if "DM2K_parameters" in globals():
            dm2k_params = dict(DM2K_parameters)
            dm2k_params.pop("DM2K__axis", None)
            dm2k_params.pop("DM2K__weights", None)
            parameters.update(dm2k_params)
        parameters.update(SVMP_parameters)
    ...
    case "pervec":
        ...
        pipeline = Pipeline([("hist", mmo.Dgm2Histogram()), ("svm", svm)])
    case "onesmk":
        diagrams = compute_diagram_pipe.fit_transform(X)
        # print(diagrams)
        smds = mmo.Dgms2SignedMeasureDistance(
            n_jobs=args.n_jobs, progress=True
        ).fit_transform(diagrams)
        # print(smds)
    ...
    case "onesmi":
        ...
        parameters = {
            ...
            "img__bandwidth": [0.01, 0.1, 1.0, 10.0, 100.0],
...
# Ensure all parameter grids are iterables for random.choice; convert None or scalars to single-item lists
for _k in list(parameters.keys()):
    _v = parameters[_k]
    if _v is None:
        parameters[_k] = [None]
    elif not isinstance(_v, (list, tuple, np.ndarray)):
        parameters[_k] = [_v]
...
n_small = min(3, len(X))
if n_small < 2:
    print("Not enough data for small fit, skipping quick checks.")
else:
    for _ in range(k):
        example_parameter = {a: choice(b) for a, b in parameters.items()}
        # Replace None choices with safe defaults for quick small-fit run
        for pk, pv in list(example_parameter.items()):
            if pv is None:
                if "resolution" in pk.lower():
                    # assign a default resolution unless the grid strategy is 'exact'
                    grid_key = pk.replace("resolution", "grid_strategy")
                    gs = example_parameter.get(grid_key, None)
                    if gs == "exact":
                        example_parameter[pk] = None
                    else:
                        example_parameter[pk] = (
                            args.out_resolution
                            if args.out_resolution is not None
                            else 50
                        )
                elif "grid_strategy" in pk.lower():
                    example_parameter[pk] = (
                        args.out_strategy if args.out_strategy is not None else "exact"
                    )
                else:
                    example_parameter[pk] = None

        # Ensure we don't pass a resolution together with an 'exact' grid strategy
        for k in list(example_parameter.keys()):
            if "resolution" in k.lower():
                grid_key = k.replace("resolution", "grid_strategy")
                # If resolution is None but grid strategy is 'exact', switch to a regular strategy
                if example_parameter.get(k, None) is None:
                    default_res = (
                        args.out_resolution if args.out_resolution is not None else 50
                    )
                    example_parameter[k] = default_res
                if example_parameter.get(grid_key, None) == "exact":
                    example_parameter[grid_key] = "regular"
        pipeline_ = clone(pipeline).set_params(**example_parameter)
        SMF.verbose = True
        print("Parameters", example_parameter)
        _X = deepcopy(X[:n_small])

        # create labels with at least two classes for the quick fit
        if n_small >= 3:
            _Y = [0, 1, 0]
        else:
            _Y = [0, 1]
        try:
            # quick fit used to precompile native code (pykeops/numba). If it
            # fails for some parameter combination, continue without failing
            # the whole script — this keeps --test robust across environments.
            pipeline_.fit(_X, _Y).score(_X, _Y)
        except Exception as e:
            print("Quick-fit failed (ignored):", repr(e))
SMF.verbose = False
print("------------ Done.", flush=True)
if args.test:
    print("------------ Done testing.")
