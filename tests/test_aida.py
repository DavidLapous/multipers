...
def test_equality():
    ...
    s_ = aida(s)
    sms = Parallel(n_jobs=-1, backend="threading")(delayed(lambda x : mp.signed_measure(x.minpres(degree), degree=degree, invariant="hilbert")[0])(x) for x in s_)
    sm2 = add_sms(sms)
