...
import numpy as np
import gudhi as gd
from multipers.diff_helper import signed_measure_indices
...
def test_h1_rips_density():
	...
	reconstructed_measure = np.asarray([[filtration_values[i][parameter] for parameter,i in enumerate(indices)] for indices in sm_indices])
...
def test_h0_rips_density():
	...
	pts, weights = sm
	_, unmappable_points = signed_measure_indices(st,pts, simplices_dimensions = [1,0])
	assert (pts[unmappable_points[:,0]][:,0]==0).all(), "Unmapped points of H0 have to be the nodes of the rips."
