...
class Simplex_tree {
 ...
 public:
  ...
  void initialize_filtration(bool ignore_infinite_values = false) {
    ...
    for (Simplex_handle sh : complex_simplex_range()) {
      if (ignore_infinite_values && std::numeric_limits<Filtration_value>::has_infinity &&
          filtration(sh) == std::numeric_limits<Filtration_value>::infinity())
        continue;
    ...
    auto fun = [&modified, this](Simplex_handle sh, int dim) -> void {
      ...
      if constexpr (SimplexTreeOptions::is_multi_parameter) {
        ...
        Boundary_simplex_iterator max_border =
            std::max_element(std::begin(boundary), std::end(boundary), [](Simplex_handle sh1, Simplex_handle sh2) {
 ...
 private:
  void make_subfiltration_non_decreasing(Simplex_handle sh) {
    ...
    for (auto sh_gosse : boundary_simplex_range(sh)) {
      ...
      stuff.push_to_least_common_upper_bound(filtration(sh_gosse));
    }
  }
 ...
 public:
   ...
   */
  std::pair<Filtration_value, Extended_simplex_type> decode_extended_filtration(Filtration_value f,
 ...
 public:
  ...
  inline static Filtration_value inf_ =
