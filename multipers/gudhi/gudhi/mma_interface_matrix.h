...
class Persistence_backend_matrix {
 ...
 public:
  ...
  class Barcode_iterator : public boost::iterator_facade<Barcode_iterator, const bar &, boost::forward_traversal_tag> {
   public:
    Barcode_iterator(const typename matrix_type::barcode_type *barcode, const std::vector<std::size_t> *inv)
   ...
   public:
    ...
    inline friend std::ostream &operator<<(std::ostream &stream, Barcode &structure) {
      ...
      for (const auto bar : structure) {
        ...
        stream << " (" << structure.barcode_->operator[](c).birth << ", " << structure.barcode_->operator[](c).death << ")";
        stream << "\n";
        ++c;
      }
      stream << "\n";
  ...
  Persistence_backend_matrix(Boundary_matrix_type &boundaries, std::vector<std::size_t> &permutation)
  ...
  inline friend std::ostream &operator<<(std::ostream &stream, Persistence_backend_matrix &structure) {
    ...
    stream << "]\n";
    stream << "permutation: ";
    for (auto i : *structure.permutation_){
      stream << i << " ";
    }
    stream << "\n";
    return stream;
